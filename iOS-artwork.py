...
def action_create(artwork_file_name, directory, create_file_name, ignore_originals):
    artwork_file = LegacyArtworkFile(artwork_file_name)
    if not artwork_file.is_legacy_supported:
        artwork_file = ModernArtworkFile(artwork_file_name)
        if not artwork_file.is_modern_supported:
    ...
    if artwork_file.is_legacy:
        if ignore_originals:
            bail("FAIL. --ignore-originals can only be used for iOS6+ artwork files. Older files can't handle this flag.")
        create_file = WriteableLegacyArtworkFile(create_file_name, artwork_file)
    else:
        create_file = WriteableModernArtworkFile(create_file_name, artwork_file, may_resize=ignore_originals)
    create_file.open()
    ...
    for artwork_image in artwork_set.iter_images():
        ...
        if (actual_width != artwork_image.width) or (actual_height != artwork_image.height):
            if ignore_originals:
                print "\n\tWARNING: the image in file %s is %d x %d in size, but the original was %d x %d." % (pil_image_name, artwork_image.width, artwork_image.height, actual_width, actual_height)
            else:
                create_file.delete()
                bail("FAIL. The image file named %s should be %d x %d in size, but is actually %d x %d.\n\n(If you really want to use this image, run the tool with --ignore-originals. Buyer beware.)" % (pil_image_name, artwork_image.width, artwork_image.height, actual_width, actual_height))
        ...
        if ignore_originals:
            is_greyscale = was_greyscale
        else:
            is_greyscale = artwork_image.is_greyscale

        if current_image_offset is None:
            current_image_offset = artwork_image.image_offset

        if (not ignore_originals) and (current_image_offset != artwork_image.image_offset):
            create_file.delete()
            bail("FAIL. Unexpected offset problems when creating a .artwork file with {0}. ({1} vs {2} : {3})".format(artwork_image.name, current_image_offset, artwork_image.image_offset, current_image_offset - artwork_image.image_offset))

        #
        # Write it
        #
        create_file.write_pil_image_at(current_image_offset, actual_width, actual_height, is_greyscale, pil_image)
        print "\timported %s" % artwork_image.retina_appropriate_name

        # Update the image offset to figure out where the next image should be written.
        current_image_offset = create_file.offset_for_next_image(current_image_offset, actual_width, actual_height, is_greyscale)
...
def main(argv):
    ...
    #
    parser = OptionParser(usage="""%prog [options] [command] [parameters]
    ...
    options:
        ...
        If used, this will prevent the tool from complaining if the
        images you're using to create a new .artwork file don't match
        their counterparts from the original in either dimension or
        in pixel depth. (Using artwork files created with this flag
        is NOT recommended,  and may cause unknown problems.)
    """)
    ...
    parser.add_option("-i", "--ignore-originals", dest="ignore_originals", help="Prevent the tool from complaining if your image dimensions or pixel depths don't match.", default=False, action='store_true')
    ...
    if command == "export":
        ...
        action_create(abs_artwork_file_name, abs_directory, abs_create_file_name, options.ignore_originals)
